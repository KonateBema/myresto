...
class HomeSlide(models.Model):
    ...
    categories = models.ManyToManyField('Category', blank=True)  # 🔥 AJOUT
...
class HistoriqueCommandeProxy(Commande):
    class Meta:
        proxy = True
...
class CashAuditLog(models.Model):
    ...
    ACTION_CHOICES = [
        ("OPEN_SHIFT", "Ouverture caisse"),
        ("CLOSE_SHIFT", "Fermeture caisse"),
    ...
    amount_before = models.DecimalField(max_digits=10, decimal_places=0, default=0)
    amount_after = models.DecimalField(max_digits=10, decimal_places=0, default=0)
    ...
    def __str__(self):
        return f"{self.get_action_display()} - {self.cashier}"
...
class CashShift(models.Model):
    ...
    opening_amount = models.DecimalField(max_digits=10, decimal_places=0, default=0)

class CashAuditProxy(CashAuditLog):
    class Meta:
        proxy = True
        verbose_name = "🧾 Audit Caisse"
        verbose_name_plural = "🧾 Audit Caisse"